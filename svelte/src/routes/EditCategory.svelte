<script>
  ...
  async function fetchCategory() {
    try {
      const res = await fetch(`http://localhost:8000/api/categories/${selectedCategoryId}`);
      if (!res.ok) throw new Error("Nepodarilo sa načítať kategóriu");
      const category = await res.json();
      name = category.name;
      existingImage = category.image ? category.image.path : "";
    } catch (error) {
  ...
  async function updateCategory() {
    ...
    const response = await fetch(`http://localhost:8000/api/categories/${selectedCategoryId}`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      console.error("Chyba pri ukladaní kategórie:", errorData);
      return;
    }
...
<div class="form-container">
  ...
  <div class="button-group">
    <button class="custom-button save-btn" on:click={updateCategory}>
      Uložiť
    </button>
